...
class Test {
    ...
    start(){
        try{
            let userInfo = {'username':"frame_sync_3","password":"123456"};
            //先登陆获取 USER TOKEN
    ...
    GatewayConfigBack(uri,err,data){
        console.log("GatewayConfigBack",data.data);
    ...
    GatewayActionMapBack(uri,err,data){
        ...
        console.log("GatewayActionMapBack success");
    ...
    GameMatchRuleBack(uri,err,data){
        console.log("GameMatchRuleBack:",data.data);
        this.gameMatchRule = data.data;
        let ws = new Ws(this.gatewayConfig,this.actionMap,this.gameMatchRule,this.token,ContentType.JSON,ProtobufType.WEBSOCKET);
        ws.Start();
    }
}
export {Test}